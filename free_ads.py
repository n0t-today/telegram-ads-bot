...
⚠️ <b>Важно:</b> Лимит {config.FREE_AD_LIMIT} символов распространяется на весь текст сообщения (включая заголовок, описание, цену, контакты и т.д.)."""
    
    await message.answer(template_text)
    await state.set_state(FreeAdStates.waiting_for_ad)


@router.message(FreeAdStates.waiting_for_ad, F.text & ~F.text.in_(MENU_TEXTS))
async def receive_free_ad_text(message: Message, state: FSMContext, bot: Bot):
    ...
    if len(ad_text) > config.FREE_AD_LIMIT:
        await message.answer(
            ...
            f"но лимит для бесплатных объявлений — {config.FREE_AD_LIMIT} символов на весь текст сообщения (включая заголовок, описание, цену, контакты и т.д.).\n\n"
    ...
    if message.media_group_id:
        ...
        if len(ad_text) > config.FREE_AD_LIMIT:
            await message.answer(
                ...
                f"но лимит — {config.FREE_AD_LIMIT} символов на весь текст сообщения (включая заголовок, описание, цену, контакты и т.д.).\n\n"
                f"Пожалуйста, сократите текст и отправьте снова."
            )
            return
