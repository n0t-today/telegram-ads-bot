...
⚠️ <b>Важно:</b> Лимит {config.PAID_AD_LIMIT} символов распространяется на весь текст сообщения (включая заголовок, описание, цену, контакты и т.д.)."""
...
async def receive_paid_ad_text(message: Message, state: FSMContext, bot: Bot):
    ...
    if len(ad_text) > config.PAID_AD_LIMIT:
        await message.answer(
            ...
            f"но лимит для платных объявлений — {config.PAID_AD_LIMIT} символов на весь текст сообщения (включая заголовок, описание, цену, контакты и т.д.).\n\n"
    ...
    if message.media_group_id:
        ...
        if len(ad_text) > config.PAID_AD_LIMIT:
            await message.answer(
                ...
                f"но лимит — {config.PAID_AD_LIMIT} символов на весь текст сообщения (включая заголовок, описание, цену, контакты и т.д.).\n\n"
                f"Пожалуйста, сократите текст и отправьте снова."
            )
            return
